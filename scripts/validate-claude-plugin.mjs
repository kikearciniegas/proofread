...
assert.equal(manifest.name, "proofread");
assert.equal(manifest.displayName, "Proofread");
...
for (const field of ["homepage", "repository"]) {
  assert.match(manifest[field], /^https:\/\//, `Claude manifest ${field} must use HTTPS`);
  assert.doesNotThrow(() => new URL(manifest[field]));
}
assert.ok(existsSync(path.join(root, "skills", "proofread", "SKILL.md")));

assert.equal(marketplace.name, "proofread");
...
console.log("Claude plugin validation passed: proofread");

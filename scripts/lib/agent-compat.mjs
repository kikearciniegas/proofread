...
export const SKILL_NAME = "proofread";
...
export function parseAgentSelection(value = "all") {
  ...
  if (unknown.length) throw new Error(`Unsupported agent(s): ${unknown.join(", ")}. Supported: ${SUPPORTED_AGENTS.join(", ")}`);
...
async function pointsToSource(target, source) {
  ...
  try { return (await realpath(target)) === (await realpath(source)); } catch {
...
export async function installPlan(plan, { copy = false, dryRun = false } = {}) {
  ...
  if (conflicts.length) throw new Error(`Refusing to overwrite existing path(s): ${conflicts.map((entry) => entry.target).join(", ")}`);
  if (dryRun) return inspected;
  for (const entry of inspected) {
    if (entry.status === "installed") continue;
    await mkdir(path.dirname(entry.target), { recursive: true });
    if (copy) await cp(entry.source, entry.target, { recursive: true, errorOnExist: true });
    else {
      const target = entry.scope === "project" ? path.relative(path.dirname(entry.target), entry.source) : entry.source;
      await symlink(target, entry.target, process.platform === "win32" ? "junction" : "dir");
    }
